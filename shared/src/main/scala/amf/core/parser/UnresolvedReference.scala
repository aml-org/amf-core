...
import amf.core.model.domain.{DomainElement, FutureDeclarationComponents, Linkable}
...
trait UnresolvedReference { this: DomainElement =>
  ...
  var ctx: Option[FutureDeclarationComponents] = None

  def withContext(c: FutureDeclarationComponents): DomainElement = {
    ctx = Some(c)
    this
  }
