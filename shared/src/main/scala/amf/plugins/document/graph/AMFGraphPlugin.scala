...
object AMFGraphPlugin extends AMFDocumentPlugin with PlatformSecrets {
  ...
  override val pipelines: Map[String, ResolutionPipeline] = Map(
      ResolutionPipeline.DEFAULT_PIPELINE -> new BasicResolutionPipeline(),
      ResolutionPipeline.EDITING_PIPELINE -> new BasicResolutionPipeline() // hack to maintain compatibility with legacy behaviour
  )
